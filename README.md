...
The publish script overwrites the `vX.Y.Z` tag and GitHub release if needed, uses the markdown file as the release body, then GitHub Actions builds and uploads the Windows `.exe` and `.msi` release files.
